...
class ModulationXYControl(object):
    ...
    def computeControl(self,
        ...
        d_obstacles = []
        self.d_obst_num = obstacles.len()
        
        velocity, angular_velocity, _, computed_target_yaw = modulationXY(cur_pos[0:2], cur_rpy[2], obst_pos[0:2], obst_orit, obst_vel[0:2], obst_ang_vel, target_pos[0:2])
    ...
    def pos_global_to_relative(pos_rob, pos_obst, theta):
        rotation_matrix = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
        return rotation_matrix.T @ (pos_rob - pos_obst)

    ################################################################################

    def pos_relative_to_global(pos_rel, theta):
        rotation_matrix = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
        return rotation_matrix @ pos_rel 

    ################################################################################

    def grad_pos_h(cur_pos_xy, obst_pos_xy, obst_orit, convex=False, return_rel=False):
        dh_dpos = np.zeros((self.d_obst_num, 2))
        dh_dpos_rel = np.zeros((self.d_obst_num, 2))

        for i in range(self.d_obst_num):
            pos = pos_global_to_relative(cur_pos_xy, obst_pos_xy[i], obst_orit[i])
            if convex:
            ...
            if return_rel:
                ...
                dh_dpos[i] = pos_relative_to_global(dh_dpos_rel, obst_orit[i])

        return dh_dpos

    ################################################################################

    def h(cur_pos_xy, obst_pos_xy, obst_orit, convex=True):
    ...
    def nominal_ctrl(cur_pos, target_pos):
    ...
    def modulationXY(cur_pos_xy, cur_yaw, obst_pos_xy, obst_orit, obst_vel_xy, obst_ang_vel, target_pos_xy):
        """
        Parameters
        ----------
        cur_pos_xy : ndarray
        ...
        u_nom = nominal_ctrl(cur_pos, target_pos)

        velocity, _= obs_avoidance_interpolation_moving(
				position = np.array(cur_pos_xy),
				initial_velocity = u_nom,
				Gamma = h(cur_pos_xy, obst_pos_xy, obst_orit, convex=self.convex) +1,
				dhdx = grad_x_h(cur_pos_xy, obst_pos_xy, obst_orit, convex=self.convex, return_rel=False),
				obs_vel = obst_vel_xy,
        ...
        target_yaw = compute_orientation(velocity, negative_velocity = False)
        rob_vel = compute_orientation_subtraction(cur_yaw, target_yaw) / self.dt

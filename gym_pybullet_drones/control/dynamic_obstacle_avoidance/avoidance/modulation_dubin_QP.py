...
def grad_x_h_circle(x, xc):
    x_rel = x-xc
    dist_square = x_rel[0]**2+x_rel[1]**2
    return x_rel, dist_square
    ...
    for n in np.arange(N_obs)[ind_obs]:
        ...
        #convert relative_velocity to normal to obstacle and tangent to obstacle directions
            ...
            for i in range(2):
                ...
                eng.cd(r'C:/Users/13681/Downloads/CBF & MPC & Modulation/code/MPC', nargout=0)
                new_velocity = eng.find_optimized_speed(current_velocity, current_orientation,new_orientation,linear_acceleration_limit, desired_velocity, obs_center[n])
                new_v = cp.Variable(1,)
                new_vxy = compute_velocity(new_orientation,new_v)
                obj = cp.Minimize(cp.sum_squares(new_v - desired_velocity))
                new_position = position + 0.5*(current_vxy+new_vxy)*dt
                new_grad,dist_square = grad_x_h_circle(new_position,obs_center[n])
                constraints = [new_v<=current_velocity+linear_acceleration_limit[1]*dt]+[new_v>=current_velocity+linear_acceleration_limit[0]*dt]+[np.dot(new_grad,new_vxy)**2<=safe_approaching_velocity**2*dist_square] 
                prob = cp.Problem(obj, constraints)
                prob.solve()
                if prob.status in (cp.OPTIMAL, cp.OPTIMAL_INACCURATE):
                    new_velocity = new_v.value

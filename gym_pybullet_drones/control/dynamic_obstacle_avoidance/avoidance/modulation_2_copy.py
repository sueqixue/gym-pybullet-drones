...
def obs_avoidance_interpolation_moving(
    ...
    for n in np.arange(N_obs)[ind_obs]:
        ...
        #convert relative_velocity to normal to obstacle and tangent to obstacle directions
            ...
            # #################################################################
            # #velocity constraint enforcement: Method old
            # obs_vel_E = np.linalg.pinv(E[:, :, n]).dot(xd_obs)
            # robot_velocity_global_E = stretched_velocity + obs_vel_E
            # robot_speed_global = np.linalg.norm(robot_velocity_global_E)
            ...
            #         P = 10
            #         for i in range(P):
            #             normal_speed = obs_vel_E[0] + (1-i/P)*stretched_velocity[0]
            #             if(normal_speed**2 +obs_vel_E[1]**2< velocity_limit**2):
            #                 stretched_velocity[0] = (1-i/P)*stretched_velocity[0]
            #                 tangent_robot1 = np.sqrt(velocity_limit**2-normal_speed**2)-obs_vel_E[1]
            #                 tangent_robot2 = -np.sqrt(velocity_limit**2-normal_speed**2)-obs_vel_E[1]
            ...
            #                 tangent_robot1 = np.sqrt(velocity_limit**2-obs_vel_E[0]**2)-obs_vel_E[1]
            #                 tangent_robot2 = -np.sqrt(velocity_limit**2-obs_vel_E[0]**2)-obs_vel_E[1]

            #         if(stretched_velocity[1]*tangent_robot1>0):
            #             stretched_velocity[1] = tangent_robot1
            #         else:
            #             stretched_velocity[1] = tangent_robot2
        ##############################################################################################
            # if(np.linalg.norm(stretched_velocity)>velocity_limit+0.1):
            #     print(stretched_velocity)
            # relative_velocity_hat[:, n] = E[:, :, n].dot(stretched_velocity)
            # relative_velocity_hat_magnitude[n] = np.linalg.norm(relative_velocity_hat[:,n])
            # if (relative_velocity_hat_magnitude[n]>velocity_limit+2):

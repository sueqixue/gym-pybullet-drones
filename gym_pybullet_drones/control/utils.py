...
from gym_pybullet_drones.control.vartools.linalg import get_orthogonal_basis
...
def get_relative_obstacle_velocity(
    ...
    for ii, it_obs in zip(range(np.sum(ind_obs)), np.arange(n_obstacles)[ind_obs]):
        if dim == 2:
            if obs_angular_velocity is None:
                xd_w = np.zeros(dim)
            else:
                # print(ii)
                # print(obs_angular_velocity)
                # print()
                # print(position)
                # print()
                # print(obs_center)
                xd_w = np.cross(
                    np.hstack(([0, 0], obs_angular_velocity[ii])),
                    np.hstack((position - np.array(obs_center[ii]), 0)),
                )
                xd_w = xd_w[0:2]
        elif dim == 3:
            if obs_angular_velocity[ii] is not None and LA.norm(
                obs_angular_velocity[ii]
            ):
                xd_w = np.cross(
                    obs_angular_velocity[ii],
                    position - obs_center[ii],
                )
            else:
                xd_w = np.zeros(dim)
        else:
        ...
        linear_velocity = obs_vel[it_obs]

        if velocity_only_in_positive_normal_direction:
            lin_vel_local = (E_orth[:, :, ii]).T.dot(obs_vel[it_obs])
            #lin_vel_local[0] is velocity normal to the obstacle boundary
            if lin_vel_local[0] < 0 and not is_boundary:
                # Obstacle is moving towards the agent
                # Yifan: The comment seems to be wrong. I think this condition is for
                # the case that obstacle is moving away from the agent
                linear_velocity = np.zeros(lin_vel_local.shape[0])
            else:
                # For safety in close region, we multiply the velocity
                lin_vel_local[0] = normal_weight_factor * lin_vel_local[0]
            ...
            #linear_velocity is velocity normal to the obstacle surfaces
        weight_linear = np.exp(-1 * (np.max([gamma_list[ii], 1]) - 1))
        if gamma_list[ii]<1.5:
            weight_linear = 1
        # if weight_linear > 0.01:
            # print(weight_linear)
        # if np.linalg.norm(linear_velocity)>14.2:
        #     print(np.linalg.norm(linear_velocity))
        xd_obs_n = weight_linear * linear_velocity + weight_angular * xd_w
